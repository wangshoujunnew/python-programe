...
def construct_decision_tree(dataset, remainedSet, targets, depth, leaf_nodes, max_depth, loss, criterion='MSE', split_points=0):
    ...
    if depth < max_depth:
        ...
        attributes = dataset.get_attributes() # 所有特征
        mse = -1
        ...
        for attribute in attributes:
            is_real_type = dataset.is_real_type_field(attribute) # 属性是否为数字
            attrValues = dataset.get_distinct_valueset(attribute) # 属性范围
            if is_real_type and split_points > 0 and len(attrValues) > split_points: # TODO split_point的含义?
                attrValues = sample(attrValues, split_points)
            for attrValue in attrValues: # 从这个特征的特征值中
                leftIdSet = []
                rightIdSet = []
                for Id in remainedSet: # TODO remainedSet含义
                ...
                leftTargets = [targets[id] for id in leftIdSet] # 顺便把对应的样本label放入到对应的targetList中
